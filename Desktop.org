...
- [[#provide-the-bw-desktop-package][Provide the bw-desktop package]]
:END:
...
(bw/system-settings-get (intern name))
#+end_src
...
(* 1024 (alist-get 'desktop/dpi bw/system-settings))
#+end_src
...
#+begin_src emacs-lisp :tangle .emacs.d/lisp/bw-desktop.el
...
#+begin_src emacs-lisp :tangle .emacs.d/lisp/bw-desktop.el
  ...
  (defun bw/setup-window-by-class ()
    (interactive)
    (pcase exwm-class-name
      ...
      ("mpv" (exwm-floating-toggle-floating)
              (bw/exwm-floating-toggle-pinned))
  ...
  (setup exwm
    ...
    (add-hook 'exwm-init-hook #'bw/exwm-init-hook)

    ;; Manipulate windows as they're created
    (add-hook 'exwm-manage-finish-hook
              (lambda ()
                ;; Send the window where it belongs
                (bw/setup-window-by-class)))
...
(defun bw/run-xmodmap ()
  (interactive)
  (start-process-shell-command "xmodmap" nil "xmodmap ~/.dotfiles/.config/i3/Xmodmap"))

(defun bw/update-wallpapers ()
  (interactive)
  (start-process-shell-command
   "feh" nil
   (format "feh --bg-scale ~/.dotfiles/backgrounds/%s" (alist-get 'desktop/background bw/system-settings))))

(setq bw/panel-process nil)
(defun bw/kill-panel ()
  (interactive)
  (when bw/panel-process
    (ignore-errors
      (kill-process bw/panel-process)))
  (setq bw/panel-process nil))

(defun bw/start-panel ()
  (interactive)
  (bw/kill-panel)
  (setq bw/panel-process (start-process-shell-command "polybar" nil "polybar panel")))

(defun bw/update-screen-layout ()
  (interactive)
...
(defun bw/configure-desktop ()
  (interactive)
    (bw/run-xmodmap)
    (bw/update-screen-layout)
    (run-at-time "2 sec" nil (lambda () (bw/update-wallpapers))))

(defun bw/on-exwm-init ()
  (bw/configure-desktop)
  (bw/start-panel))

(when bw/exwm-enabled
  ;; Configure the desktop for first load
  (add-hook 'exwm-init-hook #'bw/on-exwm-init))
...
(defun bw/send-polybar-hook (name number)
  (start-process-shell-command "polybar-msg" nil (format "polybar-msg hook %s %s" name number)))

(defun bw/update-polybar-exwm (&optional path)
  (bw/send-polybar-hook "exwm" 1)
  (bw/send-polybar-hook "exwm-path" 1))

(defun bw/update-polybar-telegram ()
  (bw/send-polybar-hook "telegram" 1))

(defun bw/polybar-exwm-workspace ()
...
(defun bw/propertized-to-polybar (buffer-name)
  (if-let* ((text (substring-no-properties buffer-name))
    ...
    (format "%%{F%s}%s%%{F-}" fg-color (bw/telega-normalize-name text))
    text))

(defun bw/polybar-telegram-chats ()
  (if (> (length tracking-buffers) 0)
    (format " %s" (string-join (mapcar 'bw/propertized-to-polybar tracking-buffers) ", "))
    ""))

(add-hook 'exwm-workspace-switch-hook #'bw/update-polybar-exwm)
(add-hook 'bufler-workspace-set-hook #'bw/update-polybar-exwm)
...
[module/exwm]
type = custom/ipc
hook-0 = emacsclient -e "(bw/polybar-exwm-workspace)" | sed -e 's/^"//' -e 's/"$//'
initial = 1
...
[module/exwm-path]
type = custom/ipc
hook-0 = emacsclient -e "(bw/polybar-exwm-workspace-path)" | sed -e 's/^"//' -e 's/"$//'
...
[module/mu4e]
type = custom/ipc
hook-0 = emacsclient -e '(bw/polybar-mail-count 500)' | sed -e 's/^"//' -e 's/"$//'
initial = 1
format-underline = ${colors.underline-1}
click-left = emacsclient -e '(bw/go-to-inbox)'

[module/telegram]
type = custom/ipc
hook-0 = emacsclient -e '(bw/polybar-telegram-chats)' | sed -e 's/^"//' -e 's/"$//'
format-padding = 3
initial = 1
...
* Provide the bw-desktop package
