...
#+PROPERTY: header-args:emacs-lisp :tangle .emacs.d/lisp/bw-mail.el
...
#+begin_src emacs-lisp
  ...
  (use-package mu4e
    ...
    (setq bw/mu4e-inbox-query
          "(maildir:/Personal/Inbox OR maildir:/Fastmail/INBOX) AND flag:unread")

    (defun bw/go-to-inbox ()
      (interactive)
      (mu4e-headers-search bw/mu4e-inbox-query))
...
#+begin_src emacs-lisp
  ...
  (use-package mu4e-alert
    ...
    (setq mu4e-alert-interesting-mail-query bw/mu4e-inbox-query)
...
Provide the =bw-mail= package so that it can be =require='d:
