...
(setq bw/is-guix-system (and (eq system-type 'gnu/linux)
                             (require 'f)
...
;; (defun bw/log-require (&rest args)
;;   (with-current-buffer (get-buffer-create "*require-log*")
;;     (insert (format "%s\n"
;;                     (file-name-nondirectory (car args))))))
;; (add-to-list 'after-load-functions #'bw/log-require)

;; Recipe is always a list
;; Install via Guix if length == 1 or :guix t is present

(defvar bw/guix-emacs-packages '()
...
(defun bw/filter-straight-recipe (recipe)
...
(setup-define :pkg
  (lambda (&rest recipe)
    (if (and bw/is-guix-system
             (or (eq (length recipe) 1)
                 (plist-get (cdr recipe) :guix)))
        `(add-to-list 'bw/guix-emacs-packages
                      ,(or (plist-get recipe :guix)
                           (concat "emacs-" (symbol-name (car recipe)))))
      `(straight-use-package ',(bw/filter-straight-recipe recipe))))
...
(setq bw/exwm-enabled (and (not bw/is-termux)
                           (eq window-system 'x)
...
(setup (:pkg evil)
  ...
  (unless bw/is-termux
    (defun bw/dont-arrow-me-bro ()
      (interactive)
      (message "Arrow keys are bad, you know?"))

    ;; Disable arrow keys in normal and visual modes
    (define-key evil-normal-state-map (kbd "<left>") 'bw/dont-arrow-me-bro)
    (define-key evil-normal-state-map (kbd "<right>") 'bw/dont-arrow-me-bro)
    (define-key evil-normal-state-map (kbd "<down>") 'bw/dont-arrow-me-bro)
    (define-key evil-normal-state-map (kbd "<up>") 'bw/dont-arrow-me-bro)
    (evil-global-set-key 'motion (kbd "<left>") 'bw/dont-arrow-me-bro)
    (evil-global-set-key 'motion (kbd "<right>") 'bw/dont-arrow-me-bro)
    (evil-global-set-key 'motion (kbd "<down>") 'bw/dont-arrow-me-bro)
    (evil-global-set-key 'motion (kbd "<up>") 'bw/dont-arrow-me-bro))
...
(setup (:pkg general)
  ...
  (general-create-definer bw/leader-key-def
    :keymaps '(normal insert visual emacs)
    :prefix "SPC"
    :global-prefix "C-SPC")

  (general-create-definer bw/ctrl-c-keys
    :prefix "C-c"))
...
(pcase system-type
  ((or 'gnu/linux 'windows-nt 'cygwin)
   (set-face-attribute 'default nil
                       :font "JetBrains Mono"
                       :weight 'light
                       :height (bw/system-settings-get 'emacs/default-face-size)))
...
(set-face-attribute 'fixed-pitch nil
                    :font "JetBrains Mono"
                    :weight 'light
                    :height (bw/system-settings-get 'emacs/fixed-face-size))

;; Set the variable pitch face
(set-face-attribute 'variable-pitch nil
                    ;; :font "Cantarell"
                    :font "Iosevka Aile"
                    :height (bw/system-settings-get 'emacs/variable-face-size)
                    :weight 'light)
...
(defun bw/show-server-edit-buffer (buffer)
  ;; TODO: Set a transient keymap to close with 'C-c C-c'
  (split-window-vertically -15)
  (other-window 1)
  (set-buffer buffer))

;; (setq server-window #'bw/show-server-edit-buffer)
...
(defun bw/org-file-jump-to-heading (org-file heading-title)
  (interactive)
...
(defun bw/org-file-show-headings (org-file)
  (interactive)
  (find-file (expand-file-name org-file))
  (counsel-org-goto)
  (org-overview)
...
(bw/leader-key-def
  ...
  "fdE" '((lambda () (interactive) (bw/org-file-show-headings "~/.dotfiles/Emacs.org")) :which-key "edit config")
  ...
  "fds" '((lambda () (interactive) (bw/org-file-jump-to-heading "~/.dotfiles/Systems.org" "Base Configuration")) :which-key "base system")
  "fdS" '((lambda () (interactive) (bw/org-file-jump-to-heading "~/.dotfiles/Systems.org" system-name)) :which-key "this system")
  "fdp" '((lambda () (interactive) (bw/org-file-jump-to-heading "~/.dotfiles/Desktop.org" "Panel via Polybar")) :which-key "polybar")
...
(defun bw/minibuffer-backward-kill (arg)
...
(setup (:pkg vertico)
  ...
  (:with-map minibuffer-local-map
    (:bind "M-h" bw/minibuffer-backward-kill))
...
(setup (:pkg consult)
  ...
  (defun bw/get-project-root ()
    (when (fboundp 'projectile-project-root)
      (projectile-project-root)))

  (:option consult-project-root-function #'bw/get-project-root
...
(defun bw/popper-window-height (window)
...
;;   "dh"  `(,(bw/dired-link "~") :which-key "Home")
;;   "dn"  `(,(bw/dired-link "~/Notes") :which-key "Notes")
;;   "do"  `(,(bw/dired-link "~/Downloads") :which-key "Downloads")
;;   "dp"  `(,(bw/dired-link "~/Pictures") :which-key "Pictures")
;;   "dv"  `(,(bw/dired-link "~/Videos") :which-key "Videos")
;;   "d."  `(,(bw/dired-link "~/.dotfiles") :which-key "dotfiles")
;;   "de"  `(,(bw/dired-link "~/.emacs.d") :which-key ".emacs.d"))
...
(bw/leader-key-def
  ...
  "os"  '(bw/counsel-rg-org-files :which-key "search notes")
...
(defun bw/org-present-prepare-slide ()
  (org-overview)
  (org-show-entry)
  (org-show-children))

(defun bw/org-present-hook ()
  ...
  (bw/org-present-prepare-slide)
  (bw/kill-panel))

(defun bw/org-present-quit-hook ()
...
(defun bw/org-present-prev ()
  (interactive)
  (org-present-prev)
  (bw/org-present-prepare-slide))

(defun bw/org-present-next ()
  (interactive)
  (org-present-next)
  (bw/org-present-prepare-slide)
...
(setup (:pkg org-present)
  (:with-map org-present-mode-keymap
    (:bind "C-c C-j" bw/org-present-next
           "C-c C-k" bw/org-present-prev))
  (:hook bw/org-present-hook)
  (:with-hook org-present-mode-quit-hook
    (:hook bw/org-present-quit-hook)))

(defvar bw/org-roam-project-template
...
(defun bw/org-roam-capture-task ()
  (interactive)
  ...
  (org-roam-capture- :node (org-roam-node-read
                     ...
                     :templates (list bw/org-roam-project-template)))

(defun my/org-roam-refresh-agenda-list ()
  (interactive)
  (setq org-agenda-files (my/org-roam-list-notes-by-tag "Project")))

(defhydra bw/org-roam-jump-menu (:hint nil)
  "
...
"
  ...
  ("m" bw/org-roam-goto-month)
  ("e" bw/org-roam-goto-year)
  ("c" nil "cancel"))

(setup (:pkg org-roam :straight t)
  (setq org-roam-v2-ack t)
  (setq bw/daily-note-filename "%<%Y-%m-%d>.org"
        bw/daily-note-header "#+title: %<%Y-%m-%d %a>\n\n[[roam:%<%Y-%B>]]\n\n")
  ...
  (:option
   ...
   `(("d" "default" entry
      "* %?"
      :if-new (file+head ,bw/daily-note-filename
                         ,bw/daily-note-header))
     ("t" "task" entry
      "* TODO %?\n  %U\n  %a\n  %i"
      :if-new (file+head+olp ,bw/daily-note-filename
                             ,bw/daily-note-header
                             ("Tasks"))
      :empty-lines 1)
     ("l" "log entry" entry
  ...
  (:global "C-c n l" org-roam-buffer-toggle
           ...
           "C-c n d" bw/org-roam-jump-menu/body
           "C-c n c" org-roam-dailies-capture-today
           "C-c n t" bw/org-roam-capture-task
...
(defun bw/switch-project-action ()
...
(setup (:pkg projectile)
  ...
  (setq projectile-switch-project-action #'bw/switch-project-action)
...
(setup (:pkg js2-mode)
  ...
  (add-hook 'js2-mode-hook #'bw/set-js-indentation)
  (add-hook 'json-mode-hook #'bw/set-js-indentation))
...
(defun bw/auto-recompile-buffer ()
  (interactive)
...
(setq bw/command-window-frame nil)

(defun bw/toggle-command-window ()
  (interactive)
  (if bw/command-window-frame
      (progn
...
(setup (:pkg obs-websocket :guix "emacs-obs-websocket-el")
  ...
  (defhydra bw/stream-keys (:exit t)
    "Stream Commands"
  ...
  (global-set-key (kbd "s-s") #'bw/stream-keys/body))
...
(defun bw/get-current-package-version ()
  (interactive)
...
(defun bw/map-line-to-status-char (line)
  (cond ((string-match "^?\\? " line) "?")))

(defun bw/get-git-status-prompt ()
  (let ((status-lines (cdr (process-lines "git" "status" "--porcelain" "-b"))))
    (seq-uniq (seq-filter 'identity (mapcar 'bw/map-line-to-status-char status-lines)))))

(defun bw/get-prompt-path ()
...
(defun bw/eshell-prompt ()
  (let ((current-branch (magit-get-current-branch))
        (package-version (bw/get-current-package-version)))
    (concat
     "\n"
     (propertize (system-name) 'face `(:foreground "#62aeed"))
     (propertize " ॐ " 'face `(:foreground "white"))
     (propertize (bw/get-prompt-path) 'face `(:foreground "#82cfd3"))
     (when current-branch
       (concat
...
(defun bw/eshell-configure ()
  ...
  (setq eshell-prompt-function      'bw/eshell-prompt
...
(setup eshell
  (add-hook 'eshell-first-time-mode-hook #'bw/eshell-configure)
...
(defun bw/esh-autosuggest-setup ()
  (require 'company)
...
(defun bw/around-tracking-add-buffer (original-func buffer &optional faces)
  (let* ((name (buffer-name buffer))
    ...
    (bw/update-polybar-telegram)
    result))

(defun bw/after-tracking-remove-buffer (buffer)
  (bw/update-polybar-telegram))

(advice-add 'tracking-add-buffer :around #'bw/around-tracking-add-buffer)
(advice-add 'tracking-remove-buffer :after #'bw/after-tracking-remove-buffer)
(advice-remove 'tracking-remove-buffer #'bw/around-tracking-remove-buffer)
...
(defun bw/before-exwm-workspace-switch (frame-or-index &optional force)
  (when (fboundp 'tracking-remove-visible-buffers)
    (when (eq exwm-workspace-current-index 0)
      (tracking-remove-visible-buffers))))

(advice-add 'exwm-workspace-switch :before #'bw/before-exwm-workspace-switch)
...
(defun bw/on-erc-track-list-changed ()
...
(setup erc
  (add-hook 'erc-track-list-changed-hook #'bw/on-erc-track-list-changed)
  (setq
      erc-nick "wallinb"
      erc-user-full-name "Bruce Wallin"
